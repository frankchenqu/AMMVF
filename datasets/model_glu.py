...
class InteractionModel(nn.Module):
    ...
    #         torch.Tensor(self.hid_dim, self.hid_dim)
    #     )
    #
...
class Predictor(nn.Module):
    def __init__(self,gat,bert, decoder,inter_att,tensor_network, device,n_fingerprint,n_layers,atom_dim=34):
        ...
        # 0.9842453868291857	0.9739130434782609	0.9385474860335196
        # 0.9827429321144406	0.9823529411764705	0.9329608938547486
        # 也对mean使用权重参数矩阵 128——64——2 dp=0.2
    ...
    #     support = torch.mm(input, self.weight)     # mm表示矩阵乘法
    ...
    # 使用了如下注意力机制之后结果变差，在此段代码中注意力实现使用的是双线性函数，存在梯度爆炸的问题
    ...
    def forward(self, compound1, compound2,adj, protein1,protein2,n_layers):
        ...
        protein1 = torch.unsqueeze(protein1, dim=0)    # (1,804,100)    # protein1 =[ batch size=1,protein len, protein_dim]
        ...
        protein1_c, compound1_p = self.decoder(protein1, compound1)     # protein1_c:(1,1,64)    # compound1_p:(1,1,64) 交互信息，decoder为MINN中的Interformer模块,都使用了mean
        ...
        compound2 = self.gat(compound2, torch.unsqueeze(adj,0),n_layers)  # 1,78,34 (取消了mean)
